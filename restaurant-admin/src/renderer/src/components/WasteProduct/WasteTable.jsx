...
const WasteTable = ({ data, searchtext = '', sortby = 'name', selectedRow, setSelectedRow, rowDoubleClick }) => {
    ...
    const filterData = useMemo(() => {
        if (data) {
            ...
            return sorted_data.filter(item => {
                if (item?.product?.name?.toLowerCase()?.includes(searchtext.toLowerCase())) {
                    return item;
                }
            })
        }
                ...
                <table className='w-full'>
                    <thead className='bg-primary sticky top-0 text-white'>
                        <tr>
                            ...
                            <th className='border px-2 py-2'>{t('Product Name')}</th>

                            <th className='border px-2 py-2'>{t('Cost')}</th>
                            <th className='border px-2 py-2'>{t('Unit')}</th>
                    ...
                    <tbody className='mt-1'>
                        {data ? filterData.map((item, index) => (
                            ...
                            >      <td className='border px-2 py-1 text-center'>{index + 1}</td>
                                ...
                                <td className='border px-2 py-1 text-right'>{numberWithCommas(item.unit)}</td>
