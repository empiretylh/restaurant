...
const VoucherView = ({
  ...
  const {
    ...
    loading,
    setLoading
  } = useContext(CashOrderContextProvider)
  ...
  const OrderItem = ({ item }) => {
      ...
      return (
        <div className="w-full flex flex-col items-center mt-1 border p-1 bg-white">
          <div className="grid grid-cols-3 w-full items-center">
            ...
            <h1 className="text-md text-black text-center">{item?.qty} x</h1>
            <div className="flex flex-row items-center">
              {item?.discount != 0 && item.discount && (
                <h1 className="text-sm ml-auto text-red-500">-{item?.discount}%</h1>
              )}

              <h1 className="text-md ml-auto">
                {numberWithCommas(computePrice(item?.discount, item?.total_price))}
              </h1>
    ...
    return (
      <Collapsible trigger={<Head />} open={isOpen}>
  ...
  const AdvancedSale = () => {
    ...
    const computeFinalPrice = useMemo(() => {
      ...
      let discount_value = parseFloat(discount)
      let discounts = isNaN(discount_value)
        ? 0
        : Math.round((parseFloat(OrignalPrice) * discount_value) / 100, 2)
	...
	useEffect(() => {
		if(settings?.autoPayment){
			setPayment(computeFinalPrice)
		}
	},[computeFinalPrice])

    const Remaingamount = useMemo(() => {
      return isNaN(parseInt(computeFinalPrice) - parseInt(payment))
        ? 0
        : parseInt(computeFinalPrice) - parseInt(payment)
      ...
      >
        ...
        <div className="max-w-md mx-auto bg-white shadow-md rounded-lg p-6 mt-10">
          <div className="w-full flex flex-row items-center mb-4">
            ...
            <icon
              className="bi bi-x-circle text-xl text-red-600 hover:text-red-800 ml-auto"
              onClick={() => setShowAdvancedSale(false)}
            ></icon>
          </div>
          <div className="mb-4">
            <label className="block text-gray-700">
              ...
              <span className="font-semibold">
                {numberWithCommas(data?.splitbill || computeTotalPrice)} Ks
              </span>
            </label>
          </div>
          <form
          ...
          >
            ...
            <div className="mb-4">
              <label className="block text-gray-700">
                Total Price:{' '}
                <span className="font-semibold">{numberWithCommas(computeFinalPrice)} Ks</span>
              </label>
            </div>
            <div className="mb-4">
              ...
              <label className="block text-gray-700">
                ...
                <span
                  className={`font-semibold ${
                    Remaingamount < 0 ? 'text-red-500' : 'text-green-500'
                  }`}
                >
                  {numberWithCommas(Remaingamount)} Ks
                </span>
              </label>
            </div>
            <div className="mb-4">
          ...
          <div className="flex flex-row items-center justify-center mt-2">
            ...
            <input
              type="checkbox"
              className=" mr-1"
              id="dontShowAdvanceSale"
              name="dontShowAdvanceSale"
              value={settings?.advancedSale}
              checked={settings?.advancedSale}
              onChange={(e) => {
                ChangeSettings(e.target.checked, 'advancedSale')
              }}
            />
            <label htmlFor="dontShowAdvanceSale" className="text-sm text-gray-700">
    ...
    <div className="bg-white shadow-lg w-full  border p-2 relative">
      ...
      <div className="w-full p-1 flex flex-row gap-2 items-center">
        ...
        <div className="flex flex-col gap-1 ml-auto ">
          ...
          {data?.splitbill || data?.totalPrice ? (
            <h1 className="font-bold text-md ml-auto text-red-600  text-right  w-full">
      ...
      <input
        ...
        checked={selectedVoucher.includes(data?.voucherid)}
        onChange={(e) => {
