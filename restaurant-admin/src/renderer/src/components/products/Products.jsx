  ...
  const ProductSubmit = (e) => {
    ...
    if (selectedRow?.id) {
      ...
      if (selectedRow.price && selectedRow.price.includes(',')) {
        ...
        let extraprice = selectedRow.price.slice(
          selectedRow.price.indexOf(',') + 1,
          selectedRow.price.length
        )
    ...
    if (form.price.value.includes(',')) {
      ...
      let extraprice = form.price.value.slice(
        form.price.value.indexOf(',') + 1,
        form.price.value.length
      )
            ...
            {selected == 'Products' ? (
              ...
              >
                ...
                <div className="flex space-x-4">
                  ...
                  <div className="flex flex-col w-full">
                  <label className="text-sm text-black font-bold mt-1">{t('Kitchen')}</label>
                  <select
                    value={selectedRow?.kitchen}
                    onChange={(e) => handleChange(e.target.value, e.target.id)}
                    className="border border-gray-300 rounded-md w-full p-2 mr-3 my-1"
                    required
                    id="kitchen"
                ...
                <div className="flex space-x-4">
                  ...
                  <div className="flex-1">
                    <label className="text-sm text-black font-bold mt-1">{t('Total Unit')}</label>
                    <input
                      value={parseInt(selectedRow?.unit) * parseInt(selectedRow?.qty)}
                      onChange={(e) => handleChange(e.target.value, e.target.id)}
                      type="number"
                      className="border border-gray-300 rounded-md w-full p-2 my-1"
                      placeholder={t('Total Unit')}
                      required
                      id="totalunit"
                      isEditable={false}
                      disabled
